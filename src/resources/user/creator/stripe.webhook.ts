...
import { PaymentStatus } from "../../../resources/room/receipts/receipts.interface";
  ...
  try {
    ...
    try {
      ...
      await Receipts.create({
        ...
        totalAmount:
          revenue * (1 - stripeService.getPlatformFeeRate(target.price)),
        ticketId,
      ...
      if (updatedRoom) {
        try {
          await emailService.sendReceiptEmail({
            paymentIntentId: pi as string,
            receiverEmail: session.customer_email as string,
            ...
            roomDate: updatedRoom.event_schedule.startDate.toISOString(),
            roomTime: updatedRoom.event_schedule.startDate.toISOString(),
            roomPrice: target.price,
          ...
          Logging.error(`Failed to send receipt email: ${err.message}`);
        }
      }
